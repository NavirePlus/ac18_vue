<template>
  <!-- Navigation-->
  <div id="navContainer" class="fixed-top hide-navbar">
    ...
    <nav class="navbar navbar-expand-lg text-uppercase" id="mainNav" style="max-width:2048px;">
      <div class="container" style="justify-content: center;text-align: center;flex-wrap:wrap;">
        ...
        <a v-on:click="moveSection" v-scroll-to="'#page-top'" class="js-scroll-trigger" href="#" style="display: block;">
          <img class="nav-logo" :src="$getImageSource('logo.webp')" />
        </a>
      </div>
    </nav>
  </div>
  <button v-on:click="toggleMenu" id="open-menu" class="font-weight-bold bg-primary text-white rounded" type="button">
    <icon-base viewBox="0 0 448 512" addClass="fab fa-bars"><icon-bars /></icon-base>
  </button>
  <div class="overlay-navigation">
    <div id="overlay-menu" role="navigation">
      <ul>
        <li class="nav-item-odd bg-primary"><a v-on:click="moveSection" v-scroll-to="'#page-top'" href="#" class="js-scroll-trigger" data-content=""><span>Home</span></a></li>
        <li class="nav-item-even"><a v-on:click="moveSection" v-scroll-to="'#about'" href="#" class="js-scroll-trigger" data-content=""><span>About</span></a></li>
        <li class="nav-item-odd bg-primary"><a v-on:click="moveSection" v-scroll-to="'#book'" href="#" class="js-scroll-trigger" data-content=""><span>Book</span></a></li>
        <li class="nav-item-even"><a v-on:click="moveSection" v-scroll-to="'#members'" href="#" class="js-scroll-trigger" data-content=""><span>Members</span></a></li>
      </ul>
    </div>
  </div>
  ...
  <header class="masthead">
    <div class="header-bg header-bg-r" :style="headerBgStyles"></div>
  </header>
</template>
...
import { toggleMenu, moveSection } from '@/assets/js/ac18'
  ...
  computed: {
    headerBgStyles() {
      ...
      if ($IMAGE_CDN_URL.startsWith('http')) {
        return {
          '--url': `url("${$IMAGE_CDN_URL}header.webp")`
        }
      } else {
        return {}
      }
...
<style scoped>
.header-bg {
  --url: url('@/assets/img/header.webp');

  background: #fff var(--url) bottom no-repeat;
